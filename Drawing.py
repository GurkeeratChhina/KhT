...
def Cobordism_1_3_ToDS(Cob): #more pythonic implementation # This is obsolete, done by CobordismToBNAlg
    """ DS is a linear combination of morphisms that are powers of D or powers of S, represented as a list of lists
        an element of DS will be refered to a ds, which is a 3 element list
        the first element is D if the morphism is a power of D, and S if it is a power of S
        the second element is the power of the corresponding morphism
    ...
    if Cob.front.top !=1 or Cob.front.bot !=3:
        print(Cob.front.top, Cob.front.bot, Cob.comps, Cob.decos)
        raise Exception("Cobordism to convert to DS is not between 4-ended tangles")
    if len(Cob.comps)==1:# saddle
        for deco in Cob.decos:
            for elem in DS:
                if elem[0] == "S" and elem[1] == 2*deco[0] +1:
                    elem[2] += (-1**deco[0])*deco[-1]
                    break
            else:
                DS.append(["S",2*deco[0] +1 , (-1**deco[0])*deco[-1]])
    ...
    if len(Cob.comps)==2:# identity/dot cobordism
        ...
        decos_DD=[["D", deco[0]+1,deco[-1]] for deco in decos_reduced if deco[j]==1] # dot cobordism
        decos_id=[["D", 0,deco[-1]] for deco in decos_no_dots if deco[0]==0] # id contribution
        decos_DH=[["D", deco[0],deco[-1]] for deco in decos_no_dots if deco[0]>0 ] # D contribution from H
        decos_SH=[["S", 2*deco[0],-deco[-1]] for deco in decos_no_dots if deco[0]>0 ] # SS contribution from H
        DS.extend(decos_DD+decos_id+decos_DH+decos_SH)
    return DS


def Draw_1_3_ChainComplex(complex, filename):
    for CLT in complex.elements:
        # print("CLT", CLT.top, CLT.bot)
        if CLT.top != 1 or CLT.bot !=3:
            raise Exception("Not a 1-3 tangle")
    
    # If a CLT is a 1-3 tangle, then the horizontal tangle is [3,2,1,0] and the vertical is [1,0,3,2]
    g = Graph()
    size = len(complex.elements)
    g.add_vertex(size)
    # print("size = " + str(size))
    ...
    for i, j in product(range(size), range(size)): #TODO: rewrite with enumerate
        if complex.morphisms[j][i].ReduceDecorations() != []: #TODO: reduce decorations earlier, not here
            g.add_edge(g.vertex(i), g.vertex(j))
            Edge_labeling[g.edge(i,j)] = ""
            for ds in Cobordism_1_3_ToDS(complex.morphisms[j][i]):
                if Edge_labeling[g.edge(i,j)] != "" and ds[2] > 0:
                    Edge_labeling[g.edge(i,j)] += "+"
                if ds[2] != 0:
                    Edge_labeling[g.edge(i,j)] += str(ds[2]) + "·" + ds[0] + str(ds[1]).translate(SUP)
    ...
    graph_draw(g, vertex_color = "black", vertex_fill_color = Vertex_labeling, vertex_size = 30,\
                edge_color = "black", edge_pen_width = 6.0, edge_text = Edge_labeling, edge_text_color = "black",\
                edge_text_distance = 10, edge_font_weight = cairo.FONT_WEIGHT_BOLD, edge_font_size = 30, \
                output_size=(600*sqrt(size), 300*sqrt(size)), edge_marker_size = 20, output="Output/" + filename)