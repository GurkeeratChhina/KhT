# tangles that admit a closure to the 2-component unlink
field = 2

html_content=""

for i in range(2,8):
    name = "("+str(i)+","+str(-i)+")-PretzelTangle"
    Tangle = Tangle.PretzelTangle(i, -i)
    #if i<=j:
    #    Tangle.twist([-2*i])
...
for i in range(2,8):
    ...
    multicurve = BNcx.to_multicurve()
    multicurve.save(name)
    html_content+=str([i,i])
    html_content += multicurve.html(name,"_Khr","hdelta",Tangle)
          
header="""---
title: 2-Component Unlink Tangles
layout: default
filename: UnlinkTangles
---
